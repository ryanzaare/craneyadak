 ...
 *
 * ⚠️ ترتیب عمدی است و یک بار برعکسش این تابع را کور کرد.
 *
 * نسخه‌ی قبلی اول کامنت‌ها را می‌زدود، بعد رشته‌ها را. رجکس خط‌کامنت
 * (`#//[^\n]*#`) نمی‌داند `//` داخل یک رشته چیز دیگری است — پس روی هر
 * URLای مثل `"https://…"` فعال می‌شد و از همان `//` تا انتهای خط را
 * حذف می‌کرد. نتیجه: یک گیومه‌ی باز بدون جفت می‌ماند، آن گیومه با
 * اولین گیومه‌ی نامرتبط *بعدی* در کل فایل جفت می‌شد، و هر چیزی بین آن
 * دو — از جمله تعریف واقعی توابع — پاک می‌شد. `cyh_to_latin_digits()`
 * دقیقاً همین‌طور از فهرست تعریف‌شده‌ها ناپدید شد و false positive داد.
 *
 * حالا رشته‌ها اول خنثی می‌شوند؛ وقتی نوبت به رجکس کامنت می‌رسد، دیگر
 * هیچ `//`ای داخل رشته باقی نمانده که با کامنت اشتباه گرفته شود.
 */
...
/**
 * محتوای بین `?>` و `<?php` را خنثی می‌کند — HTML و جاوااسکریپت خام
 * متاباکس‌های پنل، نه کد PHP.
 *
 * ⚠️ بدون این، `jQuery(...)`، `.on(...)`, `.val(...)` و بقیه‌ی فراخوانی‌های
 * جاوااسکریپت داخل `<script>` یک متاباکس، «فراخوانی تابع PHP بدون stub»
 * شمرده می‌شدند — درست همان چیزی که رفعِ باگِ ترتیب رشته/کامنت در
 * `cyh_blank()` آشکارش کرد؛ تا پیش از آن رفع، آن بخش از فایل اصلاً به
 * این‌جا نمی‌رسید. تعداد خط حفظ می‌شود تا مکان خطاهای بعدی درست بماند.
 */
function cyh_strip_inline_html( $src ) {
...
foreach ( $it as $f ) {
	...
	$src = cyh_strip_inline_html( cyh_blank( file_get_contents( $f->getPathname() ) ) );
...
/*
 * ═══════════════════════════════════════════════════════════════════════════
 * درون‌یابی رشته‌ای که کاراکتر چندبایتی آن را می‌بلعد
 * ═══════════════════════════════════════════════════════════════════════════
 * شناسه‌ی PHP قانوناً بایت‌های 0x80 تا 0xFF را می‌پذیرد. پس در رشته‌ی
 * دابل‌کوت، `"$k×$n"` به‌صورت متغیرِ **`$k×`** خوانده می‌شود، نه `$k` و بعد
 * علامت ضرب.
 *
 * این باگ در این پروژه **دو بار** رخ داد:
 *   • `"$tax→$type"`  — فلش بلعیده شد
 *   • `"$k×$n"`       — علامت ضرب بلعیده شد
 *
 * و هر دو بار بی‌صدا بود: خروجی فقط ناقص می‌شد. بار دوم هارنس هشت اخطار
 * داد و باز هم «موفق» چاپ کرد.
 *
 * علاجش یک آکولاد است: `"{$k}×{$n}"`.
 *
 * ⚠️ فقط رشته‌های **دابل‌کوت** بررسی می‌شوند. در تک‌کوت هیچ درون‌یابی‌ای
 * رخ نمی‌دهد و `%1$s` در `printf` کاملاً سالم است — بررسی سطحی‌تر، آن را
 * مثبت کاذب می‌گرفت.
 */
$interp = [];
foreach ( $plugin_files as $f ) {
	$src = file_get_contents( $f );
	// کامنت‌ها بیرون، ولی رشته‌ها دست‌نخورده — چون خودشان موضوع بررسی‌اند.
	$src = preg_replace( '#/\*[\s\S]*?\*/#', '', $src );
	$src = preg_replace( '#//[^\n]*#', '', $src );
	...
	foreach ( explode( "\n", $src ) as $n => $line ) {
		...
		foreach ( $strings[0] as $str ) {
			if ( preg_match( '/\$[A-Za-z_]\w*[\x80-\xFF]/', $str, $bad ) ) {
				$interp[] = sprintf( '%s:%d  «%s»', basename( $f ), $n + 1, $bad[0] );
			}
		}
	}
}
...
if ( $interp ) {
	fwrite( STDERR, '❌ ' . count( $interp ) . " درون‌یابی که کاراکتر چندبایتی آن را می‌بلعد:\n\n" );
	foreach ( $interp as $x ) {
		fwrite( STDERR, "   • $x\n" );
	}
	fwrite( STDERR, "\n   با آکولاد جدا کنید: \"{\$var}×\" به‌جای \"\$var×\".\n" );
	exit( 1 );
}
