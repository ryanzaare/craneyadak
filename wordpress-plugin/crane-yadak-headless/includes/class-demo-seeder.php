...
function cyh_seed_demo_products() {
	...
	foreach ( cyh_demo_products() as $item ) {
		// ------------------------------------------------------------------
		// اسلاگ صریح و انگلیسی — این خط یک باگ واقعی را رفع می‌کند.
		//
		// باگ: فیلتر `cyh_force_latin_slug()` اسلاگ لاتین را از روی برند و
		// کد فنی می‌سازد، اما آن‌ها را با `get_field()` می‌خواند. وردپرس
		// اسلاگ را *حین* `wp_insert_post()` تولید می‌کند، در حالی که این
		// سیدر فیلدهای ACF را *بعد از* درج ذخیره می‌کند. یعنی لحظه‌ی ساخت
		// اسلاگ، هنوز هیچ برند و کد فنی‌ای در دیتابیس نیست؛ فیلتر چیزی
		// پیدا نمی‌کند و وردپرس به عنوان فارسی برمی‌گردد. نتیجه:
		//     /products/کمربند-روپ-گاید-بالابر-سیم-بکسلی-پودم
		// این یک وابستگی به «ترتیب اجرا» بود، نه یک فیلتر غایب.
		//
		// راه‌حل: اسلاگ همین‌جا صریح نوشته می‌شود. قطعی است، به ترتیب
		// ذخیره‌ی هیچ فیلدی وابسته نیست، و چون کد فنی با `DEMO-` شروع
		// می‌شود اسلاگ هم با `demo-` شروع می‌شود — که دقیقاً همان چیزی است
		// که فیلتر نقشه‌ی سایت در astro.config.mjs به آن تکیه می‌کند.
		// ------------------------------------------------------------------
		$slug = strtolower( $item['sku'] );   // DEMO-POD-001 → demo-pod-001

		// بررسی تکراری‌بودن با *همان* اسلاگی که واقعاً ساخته می‌شود.
		// نسخه‌ی قبل با sanitize_title(brand.'-'.sku) چک می‌کرد که هرگز با
		// اسلاگ ساخته‌شده یکی نبود — یعنی این محافظ عملاً همیشه رد می‌شد و
		// اجرای دوباره‌ی سیدر رکورد تکراری می‌ساخت.
