...
function cyh_hub_blocks_to_acf( $blocks ) {
	...
	foreach ( (array) $blocks as $i => $b ) {
		...
		/* ── رسانه ─────────────────────────────────────────────────────
		   ⚠️ این شاخه قبلاً فقط می‌گفت «رسانه باید از پنل انتخاب شود» و
		   بلوک را دور می‌ریخت. یعنی نقشه‌های فنی، برش‌ها و نمودارهای
		   سایش — که قرار است *داخل* متن بنشینند — از مسیر ورود گروهی
		   اصلاً عبور نمی‌کردند.

		   حالا JSON تصویر را با **کلید** صدا می‌زند، نه با شناسه‌ی عددی:

		       { "type": "media",
		         "media": [ { "key": "wire-rope-cutaway",
		                      "alt": "برش عرضی سیم‌بکسل",
		                      "caption": "مغزی و شش رشته" } ] }

		   چرا کلید و نه شناسه: شناسه‌ی پیوست عددی است و در هر نصب فرق
		   می‌کند؛ یک فایل محتوا باید روی استیجینگ و پروداکشن یکسان کار
		   کند. کلید از نام فایل می‌آید و همه‌جا یکی است.

		   و چرا این تصمیم اینجاست و در اسکریپت نیست: اسکریپت نمی‌داند
		   کدام نقشه به کدام بخش تعلق دارد — کسی می‌داند که متن را
		   می‌نویسد. اسکریپت فقط *حمل* می‌کند، JSON *جا* می‌دهد. */
		if ( 'media' === $type ) {
			$items = $b['media'] ?? [];
			if ( ! is_array( $items ) || ! $items ) {
				$errors[] = "بلوک #$n از نوع media است ولی هیچ تصویری ندارد.";
				continue;
			}

			foreach ( $items as $mi => $m ) {
				$key  = isset( $m['key'] ) ? sanitize_title( (string) $m['key'] ) : '';
				$kind = in_array( ( $m['kind'] ?? 'image' ), [ 'image', 'diagram', 'video' ], true )
					? $m['kind'] : 'image';
				$vurl = esc_url_raw( (string) ( $m['video_url'] ?? ( $m['videoUrl'] ?? '' ) ) );

				// ویدیو تصویری در کتابخانه لازم ندارد؛ فقط آدرس و پوستر.
				$att = 0;
				if ( '' !== $key ) {
					$att = function_exists( 'cyh_media_by_key' ) ? cyh_media_by_key( $key ) : 0;
					if ( ! $att ) {
						/* ⚠️ کلیدِ پیدانشده **بلند** گزارش می‌شود و ردیف
						   ساخته نمی‌شود. اگر ساکت رد می‌شد، بلوک روی سایت
						   خالی می‌ماند و کسی نمی‌فهمید چرا — همان الگوی
						   شکست بی‌صدا. */
						$errors[] = "بلوک #{$n}، تصویر #" . ( (int) $mi + 1 )
							. ": کلید «{$key}» در کتابخانه‌ی رسانه نیست. "
							. 'اول تصویر را آپلود کنید (ستون «کلید محتوا» در رسانه).';
						continue;
					}
				} elseif ( 'video' !== $kind ) {
					$errors[] = "بلوک #{$n}، تصویر #" . ( (int) $mi + 1 ) . ': نه `key` دارد نه ویدیو است.';
					continue;
				}

				if ( 'video' === $kind && '' === $vurl ) {
					$errors[] = "بلوک #{$n}، تصویر #" . ( (int) $mi + 1 ) . ': نوع video است ولی `video_url` ندارد.';
					continue;
				}
				...
				$row['media'][] = [
					...
					'asset'     => $att ?: '',
					'video_url' => $vurl,
					'caption'   => cyh_hub_clean( $m['caption'] ?? '', false ),
					'alt_text'  => cyh_hub_clean( $m['alt'] ?? ( $m['alt_text'] ?? '' ), false ),
				];
			}

			if ( ! $row['media'] ) {
				$errors[] = "بلوک #$n از نوع media است ولی هیچ تصویر معتبری نداشت — رد شد.";
				continue;
			}
		}
